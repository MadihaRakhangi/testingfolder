...
df["EffectiveResistance"] = (
    df["Applied Test Voltage (V)"] / df["Measured Output Current (mA)"]
)  # floor resistance calculation
...
jf["Corrected Continuity Resistance (Ω)"] = (
    jf["Continuity Resistance (?)"] - jf["Lead Internal Resistance (?)"]
)
...
jf["Specific Conductor Resistance (MΩ/m) at 30°C"] = jf[
...
def resistance_result(
    Nom_EVolt, ATV, Eff_Floor, Dist_loc
):  # floor and wall resistance result condition
...
def resi_result(
    Type, Test_Current, Rated_OpCurrent, D_Tripped, Trip_Time
):  # residual test result condition
    if Type == "AC":
...
sf2.loc[:, "Suggested Max ELI (Ω)"] = sf2["Suggested Max ELI (Ω)"].apply(
...
def Earth_combined_graph(ef):
    ...
    colors = ["#d9534f", "#5bc0de", "#5cb85c", "#428bca"]  # Add more colors if needed
    plt.bar(
        result_counts.index, result_counts.values, color=colors
...
def main():
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(3)
    )  # Add the resistance pie chart to the document
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Add the insulation pie chart to the document
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Add the phase sequence pie chart to the document
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Add the polarity pie chart to the document
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Add the voltage drop pie chart to the document
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Generate a graph of residual current device data
    ...
    doc.add_picture(
        graph_combined, width=Inches(8), height=Inches(4)
    )  # Add the three-phase symmetry pie chart to the document
    ...
    doc.add_paragraph("ELI  TEST")
    doc = eli_test_table1(gf1, doc)
    doc.add_paragraph("\n")
    doc = eli_test_table2(gf2, doc)  # Add a table of voltage drop data to the document
